...
//ACTUALIZAR APELLIDO WEB
$(document).on("click", "#apellido", function (e) {
  e.preventDefault();
  let valor = $(this)[0].parentElement;
  apellido = $(valor).find("input").val();
  console.log(apellido);
  if (apellido.length > 1 && apellido.length < 20) {
    $.post("update.php", { apellido }, function (response) {});
  } else {
    $("#APELLIDO").val("Ingrese un dato");
...
function traerPedidos2() {
  $.ajax({
    ...
    success: function (response) {
      if (response == "null") {
        $("#en-curso").html("Sin Pedidos en curso");
      } else {
        let pedido = JSON.parse(response);
        let template8 = "";
        pedido.forEach((pedido) => {
